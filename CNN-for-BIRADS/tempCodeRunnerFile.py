...
#     parser.add_argument('--model-path', default= r'C:\Users\Pratham Jain\SisterDear\AIHC\CNN-for-BIRADS\saved_models\model.p')
#     parser.add_argument('--device-type', default="cpu")
#     parser.add_argument('--gpu-number', default=0, type=int)
#     parser.add_argument('--image-path', default= r"C:\\Users\\Pratham Jain\\SisterDear\\AIHC\\CNN-for-BIRADS\\images\\")
...
def inference(parameters, verbose=True):
    ...
    # load input images and prepare data
    datum_l_cc = utils.load_images(parameters['image_path'], 'L-CC', parameters['input_size'])
    datum_r_cc = utils.load_images(parameters['image_path'], 'R-CC', parameters['input_size'])
    datum_l_mlo = utils.load_images(parameters['image_path'], 'L-MLO', parameters['input_size'])
    datum_r_mlo = utils.load_images(parameters['image_path'], 'R-MLO', parameters['input_size'])
    x = {
        "L-CC": torch.Tensor(datum_l_cc).permute(0, 3, 1, 2).to(device),
        "L-MLO": torch.Tensor(datum_l_mlo).permute(0, 3, 1, 2).to(device),
        "R-CC": torch.Tensor(datum_r_cc).permute(0, 3, 1, 2).to(device),
...
if __name__ == "__main__":
    ...
    parser.add_argument('--gpu-number', default=0, type=int)
    parser.add_argument('--image-path', default= r"C:\\Users\\Pratham Jain\\SisterDear\\AIHC\\CNN-for-BIRADS\\images\\")
    # parser.add_argument('--image-path', default= r"C:\\Users\\Pratham Jain\\SisterDear\\AIHC\\CNN-for-BIRADS\\images\\BIRAD0_testcase\\")
    parser.add_argument('--input-size', default=(2600, 2000), type=tuple)
    args = parser.parse_args()
...
# C:\Users\Pratham Jain\SisterDear\AIHC\CNN-for-BIRADS\images\L-CC.png